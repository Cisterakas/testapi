...
async def create_student(
    ...
    query = "INSERT INTO student (user_id, student_school_id, contact, address, degree, last_school_year) VALUES (%s, %s, %s, %s, %s, %s)"
    db[1].execute(query, (user_id, student_school_id, contact, address, degree, last_school_year))
...
async def update_student(
    ...
    WHERE user_id = %s
    """
    db[1].execute(query, (student_school_id, contact, address, degree, last_school_year, user_id))
...
async def delete_student(
    ...
    try:
        # Check if the student exists
        query_check_student = "SELECT user_id FROM student WHERE user_id = %s"
        db[1].execute(query_check_student, (user_id,))
        existing_student = db[1].fetchone()
