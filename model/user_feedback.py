...
async def read_user_feedback_by_id(
    ...
    try:
        query = """
            ...
            WHERE feedback_id = %s
        """
        db[1].execute(query, (feedback_id,))
        feedback_info = db[1].fetchone()
        if feedback_info:
            return {
                "feedback_id": feedback_info[0], "user_id": feedback_info[1], 
                "request_id": feedback_info[2], "feedback_text": feedback_info[3], 
...
async def create_user_feedback(
    ...
    try:
        query = """
            ...
            VALUES (%s, %s, %s, %s, %s)
        """
        db[1].execute(query, (user_id, request_id, feedback_text, feedback_rating, feedback_date))

        # Retrieve the last inserted ID using LAST_INSERT_ID()
        db[1].execute("SELECT LAST_INSERT_ID()")
        new_feedback_id = db[1].fetchone()[0]
        db[0].commit()
...
async def update_user_feedback(
    ...
    try:
        query = """
            ...
            WHERE feedback_id = %s
        """
        db[1].execute(query, (user_id, request_id, feedback_text, feedback_rating, feedback_date, feedback_id))
...
async def delete_user_feedback(
    ...
    try:
        # Check if the user feedback exists
        query_check_feedback = "SELECT feedback_id FROM user_feedback WHERE feedback_id = %s"
        db[1].execute(query_check_feedback, (feedback_id,))
        existing_feedback = db[1].fetchone()
        ...
        # Delete the user feedback
        query_delete_feedback = "DELETE FROM user_feedback WHERE feedback_id = %s"
        db[1].execute(query_delete_feedback, (feedback_id,))
        db[0].commit()
